...
def update(dt):
    global jogador_y, oponente_y, bola_x, bola_y, bola_vx, bola_vy, placar_jogador, placar_oponente

    teclas = pygame.key.get_pressed()
    if teclas[pygame.K_UP]:
        jogador_y -= jogador_velocidade * dt
    ...
    if oponente_y + oponente_altura / 2 < bola_y:
        oponente_y += oponente_velocidade * dt
    if oponente_y + oponente_altura / 2 > bola_y:
        oponente_y -= oponente_velocidade * dt
...
while rodando:
    acumulador += min(clock.tick(240) / 1000, 0.25)
    inputs()
